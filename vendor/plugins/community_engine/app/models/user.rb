...
class User < ActiveRecord::Base
  ...
  acts_as_xapian :texts => [:login, :description]
 ...
 # validates_presence_of     :metro_area,                 :if => Proc.new { |user| user.state }
  ...
  #associations
    ...
    has_many :instigations, :class_name => "Investigation", :foreign_key => "user_id", :order => "published_at desc", :dependent => :destroy
    has_many :investigatings, :order => "created_at desc"
    has_many :investigations, :through => :investigatings, :class_name => "Investigation", :foreign_key => "investigation_id"
    ...
    has_many    :challenges, :through => :tasks, :class_name => "Challenge", :as => :undertakings
    
  #named scopes
  named_scope :recent, :order => 'users.created_at DESC'
  named_scope :not_admins, :conditions => 'users.role_id != 1'
  named_scope :admins, :conditions => 'users.role_id = 1'
  ...
  def all_investigations
    #investigations.join(instigations).sort_by{|s| s.published_at}
    instigations
  end
  
  def completed_investigations
    
    investigation_ids = self.investigatings.find(:all).collect {|x| x[:investigation_id] }
    
    Investigation.find(:all, :conditions=>['completed = 1 AND id IN (?)', investigation_ids])
  ...
  def recent_investigation_activity(page = {}, options = {})
    Activity.recent.find_without_pagination(:all, :conditions=>['investigation_id IN (?)', investigations.find(:all).collect {|x| x[:id] } ] )
  end
  ...
  def network_activity(page = {}, since = 1.week.ago)
    ...
    Activity.recent.since(since).find(:all, :conditions => ["investigation_id IN (?)", ids], :page => page)          
  ...
  def investigations_ids
    Investigating.find(:all, :conditions => ['user_id = ?', self.id]).map{|inv| inv.investigation_id }
  end
  ...
  def recommended_investigations(since = 1.week.ago)
    return [] if tags.empty?
    rec_posts = Investigation.find_tagged_with(tags.map(&:name), 
      :conditions => ['investigations.user_id != ? AND published_at > ?', self.id, since ],
      :order => 'published_at DESC',      
      :limit => 10
      )
