/* Functions for the ts_advimage plugin popup */

var preloadImg = null;
var orgImageWidth, orgImageHeight;
var baseURL = tinyMCE.baseURL.substring(0, tinyMCE.baseURL.lastIndexOf("/javascripts/tiny_mce"));

function formElement() {
  return document.forms[1];
}
function ts_check_for_duplicate(url) {
  alert("image names: " + "#{@images}");
}
function ts_onload() {
...
function ts_upload_image_path() {
  to_path = baseURL + "/simple_cms_images/create";
  return to_path;
}
function ts_insert_image(url, alt_text) {
  var formObj = formElement(); 
  //alert("image url: " + baseURL + url);
  formObj.src.value = baseURL + url; 
...
function convertURL(url, node, on_save) {
  var test = eval("tinyMCEPopup.windowOpener." + tinyMCE.settings['urlconverter_callback'] + "(url, node, on_save);");
...
function init() {
	...
	// Src browser
	html = getBrowserHTML('srcbrowser','src','image','ts_advimage');
	document.getElementById("srcbrowsercontainer").innerHTML = html;

	// Over browser
	html = getBrowserHTML('oversrcbrowser','onmouseoversrc','image','ts_advimage');
	document.getElementById("onmouseoversrccontainer").innerHTML = html;

	// Out browser
	html = getBrowserHTML('outsrcbrowser','onmouseoutsrc','image','ts_advimage');
	document.getElementById("onmouseoutsrccontainer").innerHTML = html;

	// Longdesc browser
	html = getBrowserHTML('longdescbrowser','longdesc','file','ts_advimage');
	...
	if (action == "update") {
		...
		addClassesToList('classlist', 'ts_advimage_styles');
	...
	if (tinyMCE.getParam("ts_advimage_constrain_proportions", true))
...
function insertAction() {
	...
	if (tinyMCE.getParam("accessibility_warnings")) {
		if (formObj.alt.value == "" && !confirm(tinyMCE.getLang('lang_ts_advimage_missing_alt', '', true)))
			return;
	}
...
function showPreviewImage(src, start) {
	...
	if (!start && tinyMCE.getParam("ts_advimage_update_dimensions_onchange", true))
		resetImageData();
