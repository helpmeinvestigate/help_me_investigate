class TagsController < BaseController
  ...
  def index  
    ...
    @investigation_tags = popular_tags(75, ' count DESC', 'Investigation')
    ...
    if params[:type]
      case params[:type]
        ...
        when 'Clipping', 'clippings'
          ...
          @posts, @photos, @users = [], [], []
        when 'Investigation', 'investigations'
          @pages = @clippings = Investigation.recent.find_tagged_with(tag_names, :match_all => true, :page => {:size => 10, :current => params[:page]})
          @posts, @photos, @users = [], [], []
      else
        @investigations, @clippings, @posts, @photos, @users = [], [], [], [], []
      end
    else
      ...
      @investigations = Investigation.recent.find_tagged_with(tag_names, :match_all => true, :limit => 10)
    end
  end
