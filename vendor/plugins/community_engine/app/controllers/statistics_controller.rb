...
#    @active_users_count = Activity.find(:all, :group => "user_id", :conditions => ["created_at > ?", 1.month.ago]).size
  ...
  def activities_chart
    ...
    @investigations= Activity.count(:group => "date(created_at)", :conditions => ["action = ? AND created_at > ?", 'investigation', range.days.ago ] )            
    @solutions= Activity.count(:group => "date(created_at)", :conditions => ["action = ? AND created_at > ?", 'solution', range.days.ago ] )   
    @investigatings= Activity.count(:group => "date(created_at)", :conditions => ["action = ? AND created_at > ?", 'investigating', range.days.ago ] )   
    ...
    chart.add( :series, :investigations.l, days.collect{|d| @investigations[d] || 0 } )  
    chart.add( :series, "Joined investigations", days.collect{|d| @investigatings[d] || 0 } )
    chart.add( :series, "Solutions", days.collect{|d| @solutions[d] || 0 } )
