...
function insert_uploaded_url(path) {
  //alert("You are inside insert_uploaded_url()");
  formElement().src.value = baseURL + path;
  switchType(baseURL + path);
  generatePreview();
}
...
function testonload() {
  ...
  $('media-upload').appendChild(iframe1);
  $('media_upload_form').setAttribute("action",upload_media_path());
}
...
function init() {
	...
	if (/mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(tinyMCE.getAttrib(fe, 'class'))) {
		...
		formElement().insert.value = tinyMCE.getLang('lang_update', 'Insert', true);
	}
...
function switchType(v) {
  //alert("You are inside switchType()");
	var t = getType(v), f = formElement();
  //alert("t = " + t + ", f = " + f);

	if (!t)
		return;

	selectByValue(f, 'media_type', t);
	changedType(t);

	// Update qtsrc also
	if (t == 'qt' && f.src.value.toLowerCase().indexOf('rtsp://') != -1) {
		alert(tinyMCE.getLang("lang_advmedia_qt_stream_warn"));
...
function generatePreview(c) {
  //alert("You are inside generatePreview()");
